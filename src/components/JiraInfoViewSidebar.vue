<template>
  <d-column class="jira-info-side-bar" gap elevation="2" height="100%" style="min-width: 200px">
    ...
    <d-tooltip :key="currentIssue.task.key" color="primary">
      <DProgressbar class="mx-2" v-model="currentIssue.task.fields.timespent" :max="currentIssue.task.fields.timeoriginalestimate"
                    color="primary" :show-label="false">
        <template v-slot:progress>
          <!--TODO fix progressbar in vuelize-->
          {{ currentIssue.task.fields.timespent / 3600 }}h / {{ currentIssue.task.fields.timeoriginalestimate / 3600 }}h
        </template>
      </DProgressbar>
      <template v-slot:tooltip>
        {{ secondsToTime(currentIssue.task.fields.timespent) }} / {{ secondsToTime(currentIssue.task.fields.timeoriginalestimate) }}
      </template>
    </d-tooltip>
    <d-divider class="mx-3" elevation="10"/>
    <JiraComponentLabels/>
