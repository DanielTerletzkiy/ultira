<template>
  <d-list-item :key="item.task.key" :id="item.task.key" class="list-item" :color="status.color">
    <d-row gap :wrap="false">
      <d-tooltip class="status-tooltip" position="right" :color="status.color" filled :tint="80">
        <d-avatar class="status" :color="status.color" glowing :tint="80" :size="56">
          <d-icon :name="status.icon" :color="status.color" :tint="0" :size="30"/>
        </d-avatar>
...
.list-item {
  ...
  .status {
    ...
    &::before {
      background: linear-gradient(90deg, currentColor 0%, rgba(0, 212, 255, 0) 100%);
    }
  }
